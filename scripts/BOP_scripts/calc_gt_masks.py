...
p = {
    ...
    "dataset_split_type": None,  #'env1_automated1',
    # Tolerance used in the visibility test [mm].
    "delta": 1,  # 5 for ITODD, 15 for the other datasets.
...
@click.option(
    "--datasets_path",
    default=p["datasets_path"],
    type=click.Path(exists=True, dir_okay=True, file_okay=False, path_type=Path),
    help="Default: path configured in boptoolkit.config file",
)
...
def main(datasets_path: Path, dataset_split: str, dataset_split_type: str):

    p["datasets_path"] = str(datasets_path)
    p["dataset_split"] = dataset_split
    p["dataset_split_type"] = dataset_split_type
